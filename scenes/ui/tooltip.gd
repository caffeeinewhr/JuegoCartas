...
func show_tooltip(icon: Texture, text: String,card_ui: CardUI) -> void:
	self.global_position = card_ui.global_position + Vector2(card_ui.get_rect().size.x / 2 - self.get_rect().size.x / 2, -40)
	is_visible_now = true
	if tween:
		tween.kill()
